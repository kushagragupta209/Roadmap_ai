...
from reAct_prompt.reAct import llama_reAct, llama_react_question
import json 
...
def model_call(query, syllabus):

    query = "roadmap for" + query
...
def model_call_question(query, topic, severity):
    ...
    completion = client.chat.completions.create(
            ...
            messages=[
                ...
                {
                    "role": "system",
                    "content": llama_react_question(query, topic, severity)
                }
            ]
        )
        # print(completion.choices[0].message.content)
    ...
    print(f"questions list : {raw_output}")
    data = {}
    if "Final Answer:" in raw_output:
        json_index = raw_output.index("Final Answer:")
        json_code = raw_output[json_index:]
